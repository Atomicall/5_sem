	...
	if (message == WM_DEVICECHANGE)
	{
		if (wParam == DBT_DEVICEARRIVAL)
		{
			...
			if (lpdb->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE)
			{
				...
				cout << "Устройство \"" << getFriendlyName(info) << "\" подключено!\n";
			...
			if (lpdb->dbch_devicetype == DBT_DEVTYP_VOLUME)
			{
				...
				{
					cout << "Логические действия на устройстве";
					for (int i = 0; i < 31; i++)
		...
		if (wParam == DBT_DEVICEQUERYREMOVEFAILED)
		{
			...
			{
				PDEV_BROADCAST_HANDLE info = (PDEV_BROADCAST_HANDLE)lpdb;
				cout << "Не удалось безопасно извлечб \"" << handleToFriendlyName(info->dbch_handle) << "\"\n";
		...
		if (wParam == DBT_DEVICEREMOVECOMPLETE)
		{
			...
			{
				PDEV_BROADCAST_DEVICEINTERFACE_A info = (PDEV_BROADCAST_DEVICEINTERFACE_A)lpdb;
				cout << "Устройство \"" << getFriendlyName(info) << "\" извлечено успешно"
					<< (nameToSafety((wchar_t*)info->dbcc_name) ? " и безопасно!" : ".") << '\n';
...
int main()
{
	...
	while (true)
	{
		...
		for (UsbDeviceDesc it : deviceArray)
			cout << deviceId++ << ". " << it.friendlyName << (it.removable ? "\t\tИЗВЛЕКАЕМОЕ" : "\t\tНЕИЗВЛЕКАЕМОЕ") << endl;
		...
		{
			cout << "Устройство " << static_cast<int>(choice) << " нельзя извлечь!\n";
			system("pause");
		}
	}
